...
st.markdown("""<style>
    ...
    /* Sidebar e Correção Definitiva dos Ícones/Setas */
    ...
    /* Widget Unificado iOS Horário + Clima */
...
# Auxiliar para extrair horas numéricas de strings (Ex: "4h" -> 4.0, "2.5h" -> 2.5)
...
# -----------------------------------------------------------------------------
# 5. SIDEBAR DESIGN COM LOGO OFICIAL CMPC E CARGA DE DADOS
# -----------------------------------------------------------------------------
with st.sidebar:
    # Exibição da Logo Oficial CMPC no topo da Barra Lateral
    st.image("Logo-cmpc.svg.png", use_container_width=True)
...
# -----------------------------------------------------------------------------
# 6. HEADER COM INFORMAÇÕES DE ASSINATURA DA EMPRESA E AUTORIA
# -----------------------------------------------------------------------------
...
if st.session_state.db_data is not None:
    ...
    with aba_geral:
        ...
        if not df_foco.empty:
            ...
            fig3 = px.pie(df_perf, values="Taxa Conclusão (%)", names="Disciplina", hole=0.55, color="Disciplina", color_discrete_map=DISCIPLINA_CORES)
            fig3.update_layout(template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', margin=dict(t=15,b=15,l=15,r=15))
        ...
        if exec_sel != "Selecione executante...":
            ...
            with col_pie2:
                ...
                df_horas_chart = pd.DataFrame([
                    {"Métrica": "Horas Executadas", "Valor": horas_utilizadas},
                    {"Métrica": "Horas Pendentes", "Valor": horas_restantes}
                ])
    ...
    st.warning("⬅️ Por favor, utilize o painel lateral para carregar sua planilha de programação original.")