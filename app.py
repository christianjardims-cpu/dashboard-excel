...
# Configuração da página - Layout Wide e Sidebar Expandida para UX Premium
...
# Injeção de CSS - Design System Gemini + iOS Dark Mode (Foco Extremo na UX da Sidebar)
st.markdown("""
<style>
    ...
    h1, h2, h3, h4, .gemini-title { font-family: 'Google Sans', sans-serif; }
    
    .stApp { background-color: #0E0E11; color: #E3E3E3; }
    
    /* SIDEBAR ESTILO GOOGLE GEMINI *
    ...
    /* Inputs internos da Sidebar estilo Gemini */
    ...
    .weather-card-today {
        background: linear-gradient(135deg, rgba(42, 42, 51, 0.7), rgba(26, 26, 36, 0.9));
    ...
    [data-testid="stVerticalBlock"] > div > div.stContainer {
        ...
        box-shadow: 0 12px 45px -10px rgba(0, 0, 0, 0.6);
        padding: 24px;
        margin-bottom: 20px;
    }
...
def obter_previsao_semana(referencia):
    # Base fixa realista para a região de Guaíba de acordo com o dia do ano
    previsoes_base = [
    ...
    # Rotaciona dinamicamente os valores para que o dia de hoje sempre case com a realidade da semana atual
...
dados_clima = obter_previsao_semana(hoje_dt)

# --- SIDEBAR REDESENHADA (ESTILO GEMINI AI + WEATHER HUB) ---
with st.sidebar:
    # Logo / Identidade Inteligente estilo Gemini
    st.markdown("""
        <div style='display: flex; align-items: center; gap: 10px; margin-bottom: 25px;'>
            <div style='background: linear-gradient(45deg, #1A73E8, #9333EA); width: 12px; height: 24px; border-radius: 4px;'></div>
            <span style='font-family: "Google Sans"; font-size: 1.4rem; font-weight: 600; color: #FFFFFF; letter-spacing:-0.5px;'>Guaíba Ops</span>
        </div>
    """, unsafe_allow_html=True)
    
    # CARD CLIMÁTICO CENTRAL DE GUAÍBA / RS
    st.markdown("<p style='font-size:0.8rem; color:#9AA0A6; text-transform:uppercase; font-weight:600; margin-bottom:8px; letter-spacing:0.5px;'>CLIMA EM GUAÍBA / RS</p>", unsafe_allow_html=True)
    
    # Encontra o dia atual para destaque principal
    ...
    st.markdown(f"""
        <div class="weather-card-today">
            ...
            <div style="margin-top: 14px; display: flex; gap: 12px; font-size: 0.75rem; color: #9AA0A6;">
                ...
                <span>💧 Chuva: {hoje_clima['chuva']}</span>
            </div>
        </div>
    """, unsafe_allow_html=True)
    
    # Painel Expansível Minimalista de Segunda a Sexta
    with st.expansor:
        st.markdown("<p style='font-size:0.75rem; color:#9AA0A6; margin-bottom:10px;'>Previsão Semanal (Seg - Sex)</p>", unsafe_allow_html=True)
    ...
    # Área Administrativa integrada ao design
    st.markdown("<p style='font-size:0.8rem; color:#9AA0A6; text-transform:uppercase; font-weight:600; margin-bottom:8px;'>SISTEMA</p>", unsafe_allow_html=True)
    senha_inserida = st.text_input("Acesso administrativo:", type="password", placeholder="Digite a chave...")
    
    if senha_inserida == "Programacao@2026":
        st.markdown("<br>", unsafe_allow_html=True)
        uploaded_file = st.file_uploader("Nova base (.csv ou .xlsx)", type=["csv", "xlsx"])
        if uploaded_file is not None:
            ...
            try:
                ...
                st.success("Base operacional atualizada!")
            except Exception as e: st.error(f"Erro de processamento: {e}")
    elif senha_inserida != "": 
        st.markdown("<p style='color:#FF453A; font-size:0.75rem; margin-top:4px;'>Token inválido.</p>", unsafe_allow_html=True)
...
with col_tit1:
    ...
    st.markdown(f"<p style='color: #9AA0A6; margin: 0; font-size: 1.05rem;'>Gestão Integrada de Atividades • Guaíba, RS</p>", unsafe_allow_html=True)
with col_tit2:
    st.markdown(f"<div style='text-align: right; background: rgba(30,30,36,0.5); padding: 12px; border-radius: 14px; border: 1px solid rgba(255,255,255,0.05);'><small style='color: #9AA0A6; font-weight: 500;'>FUSO BRASÍLIA</small><br><strong style='font-size: 1rem;'>{now_brasilia.strftime('%d/%m/%Y')}</strong><br><span style='color: #1A73E8; font-weight: 600; font-size: 1.2rem;'>{now_brasilia.strftime('%H:%M:%S')}</span></div>", unsafe_allow_html=True)
    ...
    for idx, row in df_filtrado_busca.iterrows():
        ...
        if novo_status in ["Pendente", "Necessita Reprogramação"]:
            motivos = [
                ...
                "Falta de Acesso", 
                "Mão de Obra", 
                "Não liberado pela operação", 
                "Condição climática não favorável", 
                "Corretivas emergênciais", 
                "Outros"
            ]
            
            idx_motivo = 0
            for m_idx, m_val in enumerate(motivos):
                if comentario_actual.startswith(m_val):
                    idx_motivo = m_idx
                    break
            ...
            if motivo_sel != "Selecione motivo...":
                ...
                novo_comentario = st.text_input(f"Detalhes adicionais ({ordem})", value=default_detalhe, key=f"det_{prefix_key}_{idx}")
                novo_comentario = f"{motivo_sel}: {novo_comentario}"
        elif novo_status == "Realizada":
            if comentario_actual:
                st.markdown(f"<small>💬 <i>Histórico anterior: {comentario_actual}</i></small>", unsafe_allow_html=True)
            novo_comentario = ""
...
if st.session_state.df is not None:
    ...
    # Alerta Inteligente de Salvamento
    if st.session_state.necessita_salvar:
        col_btn1, col_btn2 = st.columns([3, 1])
        with col_btn1:
            st.warning("⚠️ Existem alterações pendentes de salvamento em disco.")
        with col_btn2:
    ...
    # Criação das Abas Controladas de Estado Perfeito (Sem reset de página ao alternar status)
    aba_geral, aba_exec_ind, aba_exec_disc = st.tabs(
        ["📊 Acompanhamento Geral", "🛠️ Apontamento por Executante", "⚙️ Apontamento por Disciplina"]
    )
    ...
    with aba_exec_disc:
        ...
        if disc_sel:
            ...
            render_cards_com_busca(df_disc, f"disc_{disc_sel}", col_tempo)

else: 
    st.warning("⬅️ Por favor, utilize o menu lateral integrado estilo Gemini para realizar a inserção de credenciais e carregar o arquivo operacional base.")