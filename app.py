...
# -----------------------------------------------------------------------------
st.set_page_config(
    page_title="Gestão de Manutenção Global | CMPC",
    layout="wide",
    initial_sidebar_state="expanded"
)

# Reset de estado para garantir aplicação limpa do novo design fluído
if "versao_layout" not in st.session_state or st.session_state.versao_layout != "13.3_gemini_github_area_telemetria":
    st.session_state.clear()  
    st.session_state.versao_layout = "13.3_gemini_github_area_telemetria"
...
# -----------------------------------------------------------------------------
# ARCHITECTURE VISUAL & ANIMATIONS (CSS ADVANCED)
# -----------------------------------------------------------------------------
st.markdown("""
<style>
    ...
    .badge-area {
        font-family: 'Inter', sans-serif;
        font-size: 0.68rem; background: #21262d; color: #8b949e; border: 1px solid #30363d; padding: 2px 6px; border-radius: 4px; margin-left: 4px;
    }
...
# -----------------------------------------------------------------------------
# 3. TELEMETRIA METEOROLÓGICA REAL-TIME
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 5. SIDEBAR: COMPLETA COM TODAS AS ÁREAS E EXECUTANTES
# -----------------------------------------------------------------------------
with st.sidebar:
    ...
    if imagem_encontrada:
        ...
        st.markdown("<h2 style='font-size:1.4rem; font-weight:600; margin-bottom:2px;'>🏭 CMPC Global</h2>", unsafe_allow_html=True)
        
    st.markdown("<div style='margin: 14px 0; border-bottom: 1px solid #21262d;'></div>", unsafe_allow_html=True)
    st.markdown("<p style='font-size:0.68rem; color:#8b949e; font-weight:600; letter-spacing:0.8px; text-transform:uppercase; margin-bottom:12px;'>Telemetria Climática</p>", unsafe_allow_html=True)
    ...
    with st.expander("🔐 Chave Operacional", expanded=False):
        senha_inserida = st.text_input("Chave:", type="password", placeholder="Insira o token...", label_visibility="collapsed", key="v13_pass_key")
    
    if senha_inserida == "Programacao@2026":
        if "uploader_key" not in st.session_state: st.session_state.uploader_key = 9500
        uploaded_file = st.file_uploader("Atualizar Base Global:", type=["csv", "xlsx"], key=f"uploader_v13_{st.session_state.uploader_key}")
        ...
        if uploaded_file is not None:
            ...
            try:
                with st.spinner("Sincronizando Banco de Dados..."):
                    ...
                    st.session_state.db_data = carregar_dados_db()
                    st.session_state.recriar_cache = False
                st.success("Banco global sincronizado!")
                st.session_state.uploader_key += 1
                st.rerun()
            except Exception as e: st.error(f"Falha de integridade: {e}")
                
    st.markdown("<div style='margin-top:20px; font-size:0.7rem; color:#8b949e;'><center>📊 Unidade Guaíba • Todas as Áreas</center></div>", unsafe_allow_html=True)
...
with col_tit1:
    st.markdown("<h1 style='font-weight: 500; font-size: 2.1rem; margin-bottom: 2px;'>⚙️ Painel de Manutenção Integrado</h1>", unsafe_allow_html=True)
    st.markdown("<p style='color: #8b949e; margin: 0; font-size: 1rem;'>Acompanhamento de Atividades em Tempo Real • Planta CMPC Guaíba</p>", unsafe_allow_html=True)
with col_tit2:
    st.markdown(f"""
...
# -----------------------------------------------------------------------------
# 7. FILTROS E CONSTRUÇÃO DOS CARDS OPERACIONAIS COM SUPORTE DE ÁREA
# -----------------------------------------------------------------------------
...
def render_cards_operacionais(sub_df, unique_prefix):
    ...
    with col_search:
        busca = st.text_input("Filtrar ordens ou descrições:", "", placeholder="🔍 Digite o número da OMS ou termos chaves...", key=f"src_v13_{unique_prefix}", label_visibility="collapsed")
    with col_format:
        modo_exibicao = st.segmented_control("Visualização", options=["Cards", "Lista"], default="Cards", key=f"v_v13_{unique_prefix}", label_visibility="collapsed")
    ...
    for _, row in sub_df.iterrows():
        ...
        if modo_exibicao == "Cards":
            st.markdown(f"""
                <div class='os-card' style='background: {bg_card}; border-left: 4px solid {border_left_color};'>
                    <div style='display:flex; justify-content:space-between; align-items:center;'>
                        <span>
                            <strong>OMS:</strong> <code>{ordem}</code> 
                            <span class='badge-disciplina' style='background:{cor_disc};'>{row['disciplina']}</span>
                            <span class='badge-area'>{row['area']}</span>
                        </span>
            ...
            st.markdown(f"📌 <b>OMS {ordem}</b> - [{row['area']}] <span style='color:{cor_disc}; font-weight:600;'>{row['disciplina']}</span> | <span style='color:#f0f6fc;'>{desc[:90]}...</span>", unsafe_allow_html=True)
        ...
        with col_input:
            ...
            if status_em_tempo_real in ["Pendente", "Necessita Reprogramação"]:
                ...
                if ":" in comentario_atual:
                    ...
                    if partes[0] in motifs if 'motifs' in locals() else partes[0] in motivos: 
...
# -----------------------------------------------------------------------------
# 8. PROCESSAMENTO DO CORE DE ABAS (PLANTA GERAL + SELEÇÃO POR ÁREAS)
# -----------------------------------------------------------------------------
if st.session_state.db_data is not None and not st.session_state.db_data.empty:
    df_global = st.session_state.db_data.copy()
    
    # Lista todas as áreas lidas dinamicamente do banco de dados
    lista_areas_detectadas = sorted([str(x).strip() for x in df_global["area"].unique() if str(x).strip() not in ["", "nan"]])

    aba_fabrica, aba_area, aba_exec, aba_disc = st.tabs([
    ...
    # -------------------------------------------------------------------------
    # ABA 1: VISTA GERAL DA FÁBRICA
    # -------------------------------------------------------------------------
    with aba_fabrica:
        st.markdown("<h3 style='font-size:1.3rem; font-weight:500;'>Indicadores Macro Consolidados (Planta Completa)</h3>", unsafe_allow_html=True)
        df_fabrica_unicas = df_global.drop_duplicates(subset=["ordem"])
        
        tot_fabrica = len(df_fabrica_unicas)
        real_fabrica = len(df_fabrica_unicas[df_fabrica_unicas["status_execucao"] == "Realizada"])
        aderencia_fabrica = (real_fabrica / tot_fabrica * 100) if tot_fabrica > 0 else 0.0
        
        st.markdown(f"""
            <div class='kpi-container'>
                <div class='kpi-card'><div class='kpi-value'>{tot_fabrica}</div><div class='kpi-label'>Total OMS Operacionais Planta</div></div>
                <div class='kpi-card'><div class='kpi-value' style='color:#2ea44f'>{aderencia_fabrica:.1f}%</div><div class='kpi-label'>Aderência Global da Fábrica</div></div>
                <div class='kpi-card'><div class='kpi-value' style='color:#f0883e'>{tot_fabrica - real_fabrica}</div><div class='kpi-label'>Backlog Geral Ativo</div></div>
            </div>
        """, unsafe_allow_html=True)
        
        # Performance por Área cadastrada
        st.markdown("<p style='font-size:0.85rem; color:#8b949e; text-transform:uppercase; font-weight:600; letter-spacing:0.5px;'>Aderência e Volume de Ordens por Área da Fábrica</p>", unsafe_allow_html=True)
        if tot_fabrica > 0:
            resumo_areas = []
            for area_id in lista_areas_detectadas:
                df_sub_area = df_fabrica_unicas[df_fabrica_unicas["area"] == area_id]
                t_os = len(df_sub_area)
                if t_os > 0:
                    r_os = len(df_sub_area[df_sub_area["status_execucao"] == "Realizada"])
                    pct = (r_os / t_os) * 100
                    resumo_areas.append({"Área": area_id, "Ordens": t_os, "Aderência (%)": round(pct, 1)})
            
            df_resumo_areas = pd.DataFrame(resumo_areas)
            if not df_resumo_areas.empty:
                fig_areas = px.bar(df_resumo_areas, x="Área", y="Ordens", text="Aderência (%)", color="Aderência (%)",
                                   color_continuous_scale="Viridis", labels={"text": "Aderência"})
                fig_areas.update_layout(template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', margin=dict(t=15,b=15,l=10,r=10))
                st.plotly_chart(fig_areas, use_container_width=True)

    # -------------------------------------------------------------------------
    # ABA 2: ANÁLISE FILTRADA POR ÁREA DETALHADA (COM TELEMETRIA DE DISCIPLINA)
    # -------------------------------------------------------------------------
    with aba_area:
        col_sel_a1, col_sel_a2 = st.columns([1.5, 2.5])
        with col_sel_a1:
            area_selecionada = st.selectbox("Selecione a Área Industrial para Filtro Focal:", lista_areas_detectadas, key="selector_area_v13_3")
        
        df_foco = df_global[df_global["area"] == area_selecionada].copy()
        df_foco_unicas = df_foco.drop_duplicates(subset=["ordem"]).copy()
        ...
        # --- TELEMETRIA EXCLUSIVA POR DISCIPLINA DENTRO DA ÁREA ---
        df_foco_unicas["horas_num"] = df_foco_unicas["tempo_execucao"].apply(extrair_horas)
        
        Mec_os = len(df_foco_unicas[df_foco_unicas["disciplina"] == "Mecânica"])
        Mec_hrs = df_foco_unicas[df_foco_unicas["disciplina"] == "Mecânica"]["horas_num"].sum()
        
        Ele_os = len(df_foco_unicas[df_foco_unicas["disciplina"] == "Elétrica"])
        Ele_hrs = df_foco_unicas[df_foco_unicas["disciplina"] == "Elétrica"]["horas_num"].sum()
        
        Ins_os = len(df_foco_unicas[df_foco_unicas["disciplina"] == "Instrumentação"])
        Ins_hrs = df_foco_unicas[df_foco_unicas["disciplina"] == "Instrumentação"]["horas_num"].sum()
        
        st.markdown(f"""
            <div class='kpi-container'>
                <div class='kpi-card'><div class='kpi-value'>{total_os}</div><div class='kpi-label'>Total OMS ({area_selecionada})</div></div>
                ...
                <div class='kpi-card'><div class='kpi-value' style='color:#0A84FF'>{Mec_os} <span style='font-size:0.9rem; color:#8b949e;'>({int(Mec_hrs)}h)</span></div><div class='kpi-label'>⚙️ Mecânica Alocada</div></div>
                <div class='kpi-card'><div class='kpi-value' style='color:#BF5AF2'>{Ele_os} <span style='font-size:0.9rem; color:#8b949e;'>({int(Ele_hrs)}h)</span></div><div class='kpi-label'>⚡ Elétrica Alocada</div></div>
                <div class='kpi-card'><div class='kpi-value' style='color:#30D158'>{Ins_os} <span style='font-size:0.9rem; color:#8b949e;'>({int(Ins_hrs)}h)</span></div><div class='kpi-label'>🔬 Instrumentação</div></div>
            </div>
        """, unsafe_allow_html=True)
        
        # --- SELETOR DE CADA DISCIPLINA POR ÁREA ---
        st.markdown("<div style='margin: 10px 0; border-bottom: 1px solid #21262d;'></div>", unsafe_allow_html=True)
        col_lbl, col_foco_disc = st.columns([2.5, 1.5])
        with col_lbl:
            st.markdown(f"<p style='font-size:1.1rem; color:#f0f6fc; font-weight:500; margin-top:4px;'>📅 Cronograma Técnico Semanal — Área {area_selecionada}</p>", unsafe_allow_html=True)
        with col_foco_disc:
            disciplina_focal_area = st.segmented_control(
                "Filtrar Disciplina na Área:", 
                options=["Todas", "Mecânica", "Elétrica", "Instrumentação"], 
                default="Todas", 
                key=f"foco_disc_area_{area_selecionada}"
            )
        
        # Aplicando o filtro da disciplina escolhida para a visualização
        df_foco_filtrado = df_foco.copy()
        if disciplina_focal_area != "Todas":
            df_foco_filtrado = df_foco_filtrado[df_foco_filtrado["disciplina"] == disciplina_focal_area]
            
        # Parseando datas para o cronograma
        df_foco_filtrado["data_parsed"] = pd.to_datetime(df_foco_filtrado["data_inicio"], errors="coerce")
        df_foco_filtrado["dia_nome"] = df_foco_filtrado["data_parsed"].dt.day_name()
        
        if df_foco_filtrado["data_parsed"].isna().all():
            st.warning("Aviso: As atividades selecionadas não possuem um formato de data estruturado para divisão semanal.")
            render_cards_operacionais(df_foco_filtrado, f"area_full_list_{area_selecionada}")
        else:
            for eng_day, pt_day in dias_mapeamento.items():
                df_dia_area = df_foco_filtrado[df_foco_filtrado["dia_nome"] == eng_day].copy()
                if not df_dia_area.empty:
                    qtd_unicas_dia = df_dia_area["ordem"].nunique()
                    expandir_se_hoje = (pt_day.split("-")[0] == hoje_nome_pt)
                    
                    with st.expander(f"➔ {pt_day} ({qtd_unicas_dia} Atividades em {disciplina_focal_area})", expanded=expandir_se_hoje):
                        render_cards_operacionais(df_dia_area, f"area_crd_v13_3_{area_selecionada}_{eng_day}")

    # -------------------------------------------------------------------------
    # ABA 3: VISÃO POR EXECUTANTE (TODOS OS EXECUTANTES)
    # -------------------------------------------------------------------------
    with aba_exec:
        st.markdown("<h3 style='font-size:1.2rem; font-weight:500; margin-bottom:12px;'>Alocação Técnico por Executante (Toda a Fábrica)</h3>", unsafe_allow_html=True)
        lista_executantes = ["Selecione o profissional..."] + sorted(list(df_global["executante"].dropna().unique()))
        exec_sel = st.selectbox("Filtrar por técnico de campo:", lista_executantes, key="c_exec_v13", label_visibility="collapsed")
        
        if exec_sel != "Selecione o profissional...":
            df_exec = df_global[df_global["executante"] == exec_sel].copy()
            st.markdown("<br>", unsafe_allow_html=True)
    ...
    # -------------------------------------------------------------------------
    # ABA 4: LINHA POR DISCIPLINA (CRONOGRAMA TÁTICO GLOBAL)
    # -------------------------------------------------------------------------
    with aba_disc:
        ...
        disc_sel = st.selectbox("Selecione a especialidade técnica:", sorted(list(df_global["disciplina"].dropna().unique())), key="c_disc_page_v13")
        
        if disc_sel:
            df_disc = df_global[df_global["disciplina"] == disc_sel].copy()
    ...
    <div style='border: 1px dashed #30363d; border-radius: 6px; padding: 24px; text-align: center; margin-top: 20px;'>
        ...
        <small style='color: #58a6ff;'>Insira a Chave Operacional no menu lateral para fazer o upload do cronograma global da fábrica.</small>
    </div>
    """, unsafe_allow_html=True)