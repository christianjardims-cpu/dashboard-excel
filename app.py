...
# --- CSS INTEGRADO (ORIGINAL + GEMINI STYLE) ---
st.markdown("""
<style>
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&display=swap');
    .stApp { background-color: #000000; color: #FFFFFF; }
    [data-testid="stSidebar"] { background-color: #0F0F0F; border-right: 1px solid #282828; padding: 24px 16px; }
...
with st.sidebar:
    ...
    if senha_inserida == "Programacao@2026":
        ...
        if uploaded_file:
            # (Seu código original de leitura de arquivo aqui...)
            st.success("Base atualizada!")

# --- LÓGICA DE ATUALIZAÇÃO COM TOAST ---
def processar_status(idx, novo_status, novo_comentario):
    st.session_state.df.loc[idx, "Status_Execucao"] = novo_status
    st.session_state.df.loc[idx, "Comentario"] = str(novo_comentario)
    salvar_dados(st.session_state.df)
    st.toast(f"Ordem atualizada com sucesso!", icon="✅")

# --- RESTANTE DO SEU CÓDIGO ORIGINAL ---
# (Cole aqui todo o seu bloco "if df is not None:" e os "tabs")
import pandas as pd
import streamlit as st
import plotly.express as px
import plotly.graph_objects as go
...
# Mapeamento de Cores Definitivo: Pendente (Vermelho), Realizada (Verde), Necessita Reprogramação (Laranja)
COLOR_MAP = {
...
def render_cards_com_busca(sub_df, prefix_key, local_col_tempo):
    ...
    for idx, row in df_filtrado_busca.iterrows():
        ...
        comentario_actual = str(row_actual.get("Comentario", ""))
        if comentario_actual == "nan" or comentario_actual == "None": 
            comentario_actual = ""
        ...
        if novo_status in ["Pendente", "Necessita Reprogramação"]:
            ...
            if motivo_sel != "Selecione motivo...":
                novo_comentario = st.text_input(f"Detalhes adicionais ({ordem})", value=comentario_actual if "Selecione" not in comentario_actual else "", key=f"det_{prefix_key}_{idx}")
                novo_comentario = f"{motivo_sel}: {novo_comentario}"
        elif novo_status == "Realizada":
            if comentario_actual:
                st.markdown(f"<small>💬 <i>Histórico: {comentario_actual}</i></small>", unsafe_allow_html=True)
            novo_comentario = ""
