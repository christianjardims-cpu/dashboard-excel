...
if "versao_layout" not in st.session_state or st.session_state.versao_layout != "13.0_gemini_github_global":
    st.session_state.clear()  
    st.session_state.versao_layout = "13.0_gemini_github_global"
    ...
    for _, row in sub_df.iterrows():
        ...
        with col_input:
            ...
            if status_em_tempo_real in ["Pendente", "Necessita Reprogramação"]:
                ...
                elif comentario_atual in motifs: 
...
if st.session_state.db_data is not None and not st.session_state.db_data.empty:
    ...
    # -------------------------------------------------------------------------
    # ABA 2: ANÁLISE FILTRADA POR ÁREA DETALHADA
    # -------------------------------------------------------------------------
    with aba_area:
        col_sel_a1, col_sel_a2 = st.columns([1.5, 2.5])
        with col_sel_a1:
            area_selecionada = st.selectbox("Selecione a Área Industrial para Filtro Focal:", lista_areas_detectadas, key="selector_area_v13")
        ...
        st.markdown(f"""
            <div class='kpi-container'>
                <div class='kpi-card'><div class='kpi-value'>{total_os}</div><div class='kpi-label'>Ordens na Área: {area_selecionada}</div></div>
                ...
                <div class='kpi-card'><div class='kpi-value' style='color:#f0883e'>{total_os - realizadas}</div><div class='kpi-label'>Backlog da Área</div></div>
            </div>
        """, unsafe_allow_html=True)
    ...
    # -------------------------------------------------------------------------
    with aba_exec:
        st.markdown("<h3 style='font-size:1.2rem; font-weight:500; margin-bottom:12px;'>Alocação Técnica por Executante (Toda a Fábrica)</h3>", unsafe_allow_html=True)
    ...
    # -------------------------------------------------------------------------
    # ABA 4: LINHA POR DISCIPLINA (CRONOGRAMA TÁTICO)
    # -------------------------------------------------------------------------
    with aba_disc:
