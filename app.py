    ...
    if senha_inserida == "Programacao@2026":
        ...
        if uploaded_file is not None:
            ...
            try:
                ...
                st.success("Base updated successfully!")
            except Exception as e: st.error(f"Erro ao ler arquivo: {e}")
    elif senha_inserida != "": st.error("Senha incorreta.")

def render_cards_com_busca(sub_df, prefix_key, local_col_tempo):
    busca_termo = st.text_input(f"🔍 Pesquisar Ordem ou Descrição ({prefix_key})", "", placeholder="Digite o número da Ordem ou palavra-chave...", key=f"search_box_{prefix_key}")
    st.markdown("<br>", unsafe_allow_html=True)
...
if st.session_state.df is not None:
    ...
    # Alerta persistente e flutuante caso existam dados modificados
    ...
    # Função callback para monitorar a troca manual de abas pelo usuário
    def track_tab_change():
        if "main_tab_navigator" in st.session_state:
            tab_mapping = {"📊 Acompanhamento Geral": 0, "🛠️ Apontamento por Executante": 1, "⚙️ Apontamento por Disciplina": 2}
            st.session_state.active_tab = tab_mapping.get(st.session_state.main_tab_navigator, 0)

    # Abas controladas programaticamente
    ...
    # Renderização Condicional do Conteúdo baseado na Aba Ativa
    with aba_geral:
    ...
    with aba_exec_ind:
        ...
        if exec_sel != "Selecione...":
            ...
            for dia in dias_da_semana_ordem:
                ...
                if not df_dia.empty:
                    st.markdown(f"### 📅 {dias_pt[dia]} <span style='font-size: 1.1rem; color: #8E8E93; font-weight: 400;'>({len(df_dia)} ordens)</span>", unsafe_allow_html=True)
                    st.markdown("<br>", unsafe_allow_html=True)
