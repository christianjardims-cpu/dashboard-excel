...
def atualizar_historico(df):
    ...
    total = len(df)
    realizadas = len(df[df["Status_Execucao"] == "Realizada"])
    taxa = (realizadas / total * 100) if total > 0 else 0
    
    novo_registro = pd.DataFrame([{"Data_Consolidacao": hoje, "Taxa_Realizacao": taxa}])
    if os.path.exists(ARQUIVO_HISTORICO):
        hist = pd.read_csv(ARQUIVO_HISTORICO)
        hist = hist[hist["Data_Consolidacao"] != hoje]
...
if df is not None:
    ...
    with aba_geral:
        ...
        if os.path.exists(ARQUIVO_HISTORICO):
            ...
            fig_evolucao = px.line(hist_df, x="Data_Consolidacao", y="Taxa_Realizacao", markers=True, 
                                   labels={"Taxa_Realizacao": "Taxa de Realização (%)", "Data_Consolidacao": "Data"})
    ...
    with aba_exec_disc:
        ...
        # Alterado para ler estritamente de df_foco (Caldeira e Energia)
...
else: st.warning("⬅️ Por favor, insira a senha e faça o upload do arquivo base no menu lateral para inicializar o painel.")