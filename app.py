        ...
        with col_input:
            ...
            if status_em_tempo_real in ["Pendente", "Necessita Reprogramação"]:
                ...
                if ":" in comentario_atual:
                    ...
                    if partes[0] in motivos: motivo_inicial, detalhe_inicial = partes[0], partes[1].strip()
...
# -----------------------------------------------------------------------------
# 7. SISTEMA DE ABAS CONECTADO COM INJEÇÃO ESTÁTICA SEGURA
# -----------------------------------------------------------------------------
if st.session_state.db_data is not None and not st.session_state.db_data.empty:
    df_geral = st.session_state.db_data
    
    # Coleta as áreas existentes no arquivo e injeta estaticamente a LINHA DE FIBRAS
    ...
    with aba_exec:
        ...
        with col_f2:
            ...
            # Garantia estática do executante VITOR POHEN na Linha de Fibras
        ...
        if exec_sel != "Selecione executante...":
            ...
            if df_exec.empty and exec_sel == "VITOR POHEN":
                st.info(f"O executante {exec_sel} foi adicionado com sucesso à área {area_exec_sel}. Nenhuma ordem de serviço pendente no banco para ele até o momento.")
            else:
                st.divider()
    ...
    with aba_disc:
        ...
        if disc_sel:
            ...
            if df_disc.empty and area_disc_sel == "LINHA DE FIBRAS":
                st.info(f"Área {area_disc_sel} vinculada à disciplina {disc_sel}. Aguardando importação de ordens planejadas.")
            else:
    ...
    st.warning("⬅️ Banco de dados desatualizado ou vazio. Insira a Chave Operacional no menu lateral para carregar sua planilha.")