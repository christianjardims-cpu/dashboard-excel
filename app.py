...
# -----------------------------------------------------------------------------
# 1. CONFIGURAÇÕES INICIAIS E ARQUITETURA VISUAL
# -----------------------------------------------------------------------------
st.set_page_config(
...
# Controle estrito de cache
if "versao_layout" not in st.session_state or st.session_state.versao_layout != "7.0_jornada_label":
    st.session_state.clear()  
    st.session_state.versao_layout = "7.0_jornada_label"
...
# CSS Geral do Painel (Otimizado com as classes da barra de progresso)
st.markdown("""<style>
    ...
    /* Widget de Horário iOS Customizado com Identificação de Jornada */
    ...
    /* Identificação Precisa da Barra de Progresso */
    ...
    .ios-progress-container { background: rgba(255, 255, 255, 0.05); border-radius: 4px; height: 6px; width: 100%; overflow: hidden; margin-bottom: 2px; }
...
# -----------------------------------------------------------------------------
# 3. TELEMETRIA METEOROLÓGICA (OPEN-METEO)
# -----------------------------------------------------------------------------
@st.cache_data(ttl=600)
def obter_dados_meteorologicos_puros_v7():
    try:
        ...
        cronograma_fallback = [
            {"nome": "Segunda", "status": "☁️", "temp": "14°C"},
            {"nome": "Terça", "status": "⛅", "temp": "15°C"},
            {"nome": "Quarta", "status": "☁️", "temp": "16°C"},
            {"nome": "Quinta", "status": "☁️", "temp": "18°C"},
            {"nome": "Sexta", "status": "⛅", "temp": "14°C"}
        ]
        return "14°C", "☁️ Encoberto", cronograma_fallback

temp_real, status_real, dados_clima = obter_dados_meteorologicos_puros_v7()
...
# Cálculo preciso do progresso de horas comerciais/trabalho do dia (Base: 08:00 às 17:00)
...
pct_jornada = (minutos_atuais_jornada / minutos_totais_trabalho) * 100
pct_jornada = min(100.0, max(0.0, pct_jornada))
...
with st.sidebar:
    ...
    st.write("---")
    st.caption("☁️ PREVISÃO METEOROLÓGICA (GUAÍBA)")
    
    with st.container(border=True):
        for info in dados_clima:
            col1, col2, col3 = st.columns([2.5, 1, 1.2])
            if info["nome"] == hoje_nome_pt:
                col1.write(f"**{info['nome']} (Hoje)**")
            else:
                col1.write(info["nome"])
            col2.write(info["status"].split(" ")[0])
            col3.write(f"**{info['temp']}**")
                
    st.write("")

    st.caption("🔒 CONFIGURAÇÃO INTERNA")
    senha_inserida = st.text_input("Chave operacional:", type="password", placeholder="Insira a senha...", label_visibility="collapsed", key="v7_stable_pass_key")
    
    if senha_inserida == "Programacao@2026":
        if "uploader_key" not in st.session_state: st.session_state.uploader_key = 5000
        uploaded_file = st.file_uploader("Upload da Programação:", type=["csv", "xlsx"], key=f"uploader_v7_{st.session_state.uploader_key}")
...
# -----------------------------------------------------------------------------
# 6. HEADER: CARD DE HORÁRIO COM IDENTIFICAÇÃO PRECISA DA JORNADA DE TRABALHO
# -----------------------------------------------------------------------------
...
def render_cards_operacionais(sub_df, unique_prefix):
    busca = st.text_input("🔍 Filtrar Ordens", "", placeholder="Filtrar...", key=f"src_v7_{unique_prefix}")
    if busca:
    ...
    modo_exibicao = st.segmented_control("Formato:", options=["Cards Detalhes", "Lista Compacta"], default="Cards Detalhes", key=f"v_v7_{unique_prefix}")
    ...
    for _, row in sub_df.iterrows():
        ...
        with col_input:
            ...
            if status_em_tempo_real in ["Pendente", "Necessita Reprogramação"]:
                ...
                with col_sel:
                    st.selectbox(
                        "Justificativa", ["Selecione motivo..."] + motifs, index=(["Selecione motivo..."] + motifs).index(motivo_inicial),
...
# -----------------------------------------------------------------------------
# 8. SISTEMA DE ABAS (SELETOR DE GRÁFICO E DIVISÃO DE DIAS)
# -----------------------------------------------------------------------------
if st.session_state.db_data is not None:
    ...
    with aba_geral:
        ...
        if not df_foco.empty:
            ...
            disc_alvo = st.selectbox("Escolha a disciplina que deseja acompanhar no gráfico:", lista_discs_existentes, key="seletor_reativo_final_v7")
            ...
            st.plotly_chart(fig_barras_horizontal, use_container_width=True, key="graph_v7_core_engine")
    ...
    with aba_exec:
        ...
        exec_sel = st.selectbox("Escolha o Executante da Área:", lista_executantes, key="c_exec_v7")
    ...
    with aba_disc:
        ...
        disc_sel = st.selectbox("Selecione a Disciplina para Filtragem Semanal:", sorted(list(df_foco["disciplina"].dropna().unique())), key="c_disc_page_v7_def")
                ...
                for eng_day, pt_day in dias_mapeamento.items():
                    ...
                    if not df_dia_disc.empty:
                        with st.expander(f"📅 {pt_day} — {disc_sel} ({len(df_dia_disc)} Ordens)", expanded=(pt_day.split("-")[0] == hoje_nome_pt)):
                            render_cards_operacionais(df_dia_disc, f"disc_crd_v7_{disc_sel}_{eng_day}")
else:
