...
# Força o reset completo de qualquer estado anterior salvando uma nova assinatura de versão (v11)
if "versao_layout" not in st.session_state or st.session_state.versao_layout != "11.0_gemini_guaiba_force":
    st.session_state.clear()  
    st.session_state.versao_layout = "11.0_gemini_guaiba_force"
...
# CSS Framework customizado inspirado nas interfaces fluidas do Gemini da Google
st.markdown("""<style>
    ...
    /* CARD DE CLIMA ESTILO GEMINI - GLASSMORPHISM AVANÇADO */
...
def obter_dados_meteorologicos_puros_v11():
    try:
        # Coordenadas estritas para Guaíba - RS (-30.1139, -51.3250)
...
# -----------------------------------------------------------------------------
# 5. SIDEBAR: MONITORAMENTO DE CLIMA MODERNIZADO ESTILO GEMINI
# -----------------------------------------------------------------------------
with st.sidebar:
    ...
    # Bloco Gemini Core para Meteorologia Semanal
    ...
    senha_inserida = st.text_input("Chave operacional:", type="password", placeholder="Insira a senha...", label_visibility="collapsed", key="v11_stable_pass_key")
    
    if senha_inserida == "Programacao@2026":
        if "uploader_key" not in st.session_state: st.session_state.uploader_key = 9000
        uploaded_file = st.file_uploader("Upload da Programação:", type=["csv", "xlsx"], key=f"uploader_v11_{st.session_state.uploader_key}")
...
# -----------------------------------------------------------------------------
# 6. HEADER: CARD DE HORÁRIO COM GEOLOCALIZAÇÃO EXATA FORÇADA (GUAÍBA - RS)
# -----------------------------------------------------------------------------
...
def render_cards_operacionais(sub_df, unique_prefix):
    ...
    busca = st.text_input("🔍 Filtrar Ordens", "", placeholder="Filtrar...", key=f"src_v11_{unique_prefix}")
    if busca:
    ...
    modo_exibicao = st.segmented_control("Formato:", options=["Cards Detalhes", "Lista Compacta"], default="Cards Detalhes", key=f"v_v11_{unique_prefix}")
...
if st.session_state.db_data is not None:
    ...
    with aba_geral:
        ...
        if not df_foco_unicas.empty:
            ...
            disc_alvo = st.selectbox("Escolha a disciplina que deseja acompanhar no gráfico:", lista_discs_existentes, key="seletor_reativo_final_v11")
            ...
            st.plotly_chart(fig_barras_horizontal, use_container_width=True, key="graph_v11_core_engine")
    ...
    with aba_exec:
        ...
        exec_sel = st.selectbox("Escolha o Executante da Área:", lista_executantes, key="c_exec_v11")
    ...
    with aba_disc:
        ...
        disc_sel = st.selectbox("Selecione a Disciplina para Filtragem Semanal:", sorted(list(df_foco["disciplina"].dropna().unique())), key="c_disc_page_v11_def")
                ...
                for eng_day, pt_day in dias_mapeamento.items():
                    ...
                    if not df_dia_disc.empty:
                        ...
                        with st.expander(f"📅 {pt_day} — {disc_sel} ({qtd_unicas_disc} Ordens)", expanded=(pt_day.split("-")[0] == hoje_nome_pt)):
                            render_cards_operacionais(df_dia_disc, f"disc_crd_v11_{disc_sel}_{eng_day}")
else:
