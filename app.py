...
if "versao_layout" not in st.session_state:
    ...
    st.session_state.versao_layout = "3.2_gemini_stable"
...
st.markdown("""<style>
    ...
    div[data-testid="stselectbox"] > div > div { background-color: #1A1B22;
        border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 12px; color: white;
    }
    ...
    .ios-date-badge { background: rgba(10, 132, 255, 0.12); color: #0A84FF; font-size: 0.75rem; font-weight: 600; padding: 4px 10px;
        border-radius: 20px; }
    .ios-weather-row { display: flex; justify-content: space-between; align-items: center; font-size: 0.85rem; border-top: 1px solid rgba(255,255,255,0.06);
        padding-top: 6px; margin-top: 4px; }
    .ios-progress-container { background: rgba(255, 255, 255, 0.05); border-radius: 4px; height: 5px;
    ...
    .gemini-sidebar-header { font-family: 'Google Sans', sans-serif; font-size: 0.85rem; font-weight: 600; color: #8AB4F8; letter-spacing: 0.5px; margin-bottom: 14px;
        display: flex; align-items: center; gap: 8px; }
    .gemini-sidebar-row { display: flex; justify-content: space-between; align-items: center;
        padding: 10px 8px; border-bottom: 1px solid rgba(255, 255, 255, 0.04); font-size: 0.85rem; border-radius: 8px;
    }
    ...
    .os-card { border-radius: 14px;
        padding: 18px; margin-bottom: 16px; box-shadow: 0 4px 20px rgba(0,0,0,0.2); transition: transform 0.2s ease; position: relative;
    }
    .os-card:hover { transform: translateY(-2px); }
    .badge-disciplina { font-size: 0.7rem; font-weight: 600;
        padding: 3px 8px; border-radius: 8px; text-transform: uppercase; margin-left: 8px; color: #FFF;
    }
    
    /* Painéis de KPI */
    .kpi-container { display: flex;
        gap: 1.5rem; margin-bottom: 2rem; margin-top: 1rem; }
    .kpi-card { background: rgba(30, 30, 36, 0.5);
        border: 1px solid rgba(255, 255, 255, 0.06); border-radius: 18px; padding: 1.5rem; flex: 1; text-align: center;
    }
    .kpi-value { font-size: 2.3rem; font-weight: 700; color: #1A73E8; }
    .kpi-label { font-size: 0.85rem;
        color: #9AA0A6; text-transform: uppercase; font-weight: 500; }
    ...
    /* ESTILIZAÇÃO DO CONTROLADOR DE STATUS REATIVO (SEM EMOJIS, COMPATÍVEL COM COR DO BALÃO) */
    div[data-testid="stHorizontalBlock"] div[class*="st-emotion-cache"] div[role="radiogroup"] {
        gap: 6px !important;
    }
...
# -----------------------------------------------------------------------------
# 5. SIDEBAR: LAYOUT GEMINI COM CORREÇÃO DE RENDERIZAÇÃO HTML (`unsafe_allow_html`)
# -----------------------------------------------------------------------------
with st.sidebar:
    ...
    # WIDGET DE CLIMA PREMIUM DE SEGUNDA A SEXTA (ESTILO GOOGLE GEMINI) - CORRIGIDO
    html_gemini_weather = """
...
def render_cards_operacionais(sub_df, unique_prefix):
    ...
    for _, row in sub_df.iterrows():
        ...
        # Identificador único higienizado para injeção de CSS restrito a este card
        css_id = f"ctrl_{unique_prefix}_{id_reg}".replace(".", "_").replace("/", "_").replace("-", "_")
        ...
        # Injeção de CSS Dinâmico Avançado para casar a cor do Seletor Ativo com a borda/fundo do Balão
        st.markdown(f"""
            <style>
                /* Alvos específicos para colorir o controle segmentado ativo baseado no status atual do banco */
                div[data-parent-as-item="{css_id}"] div[role="radiogroup"] div[data-testid="stSegmentedControlItem"] button[aria-checked="true"] {{
                    background-color: {border_left_color} !important;
                    color: #000000 !important;
                    font-weight: 600 !important;
                }}
                div[data-parent-as-item="{css_id}"] div[role="radiogroup"] div[data-testid="stSegmentedControlItem"] button {{
        ...
        with col_rad:
            # Envelopado em uma div identificável para aplicação segura do CSS customizado
            st.markdown(f'<div data-parent-as-item="{css_id}">', unsafe_allow_html=True)
            st.segmented_control(
                "Status", 
                options=["Pendente", "Realizada", "Necessita Reprogramação"], 
                default=status_atual, 
                key=f"st_{unique_prefix}_{id_reg}", 
