...
if df is not None:
    # Limpar espaços em branco dos nomes de colunas
    ...
    if "Área" not in df.columns:
        ...
        if executante_selecionado != "Selecione...":
            ...
            if total > 0:
                ...
                # Correção do Gráfico: Criado um DataFrame explícito para leitura segura
                df_grafico = pd.Series(status_list).value_counts().reset_index()
                df_grafico.columns = ["Status", "Quantidade"]
                
                # Exibe o gráfico de barras usando a coluna "Status" como índice
