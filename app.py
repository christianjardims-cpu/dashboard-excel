...
# Força o reset completo de qualquer estado anterior mudando a assinatura da versão (v15)
if "versao_layout" not in st.session_state or st.session_state.versao_layout != "15.0_colors_and_arrows_fix":
    st.session_state.clear()  
    st.session_state.versao_layout = "15.0_colors_and_arrows_fix"
...
# CSS Framework customizado limpo (Sem quebrar os expanders nativos)
st.markdown("""<style>
...
def obter_dados_meteorologicos_puros_v15():
    try:
...
temp_real, status_real, dados_clima = obter_dados_meteorologicos_puros_v15()
...
with st.sidebar:
    ...
    senha_inserida = st.text_input("Chave operacional:", type="password", placeholder="Insira a senha...", label_visibility="collapsed", key="v15_stable_pass_key")
    
    if senha_inserida == "Programacao@2026":
        if "uploader_key" not in st.session_state: st.session_state.uploader_key = 13000
        uploaded_file = st.file_uploader("Upload da Programação:", type=["csv", "xlsx"], key=f"uploader_v15_{st.session_state.uploader_key}")
...
# -----------------------------------------------------------------------------
# 6. HEADER: CARD DE HORÁRIO COM REFRESH APENAS DE HORAS E MINUTOS
# -----------------------------------------------------------------------------
...
# Bloco dinâmico isolado para atualização em tempo real (Apenas Horas e Minutos Garantidos)
@st.fragment(run_every=10)
def renderizar_relogio_realtime_hm_strict():
    ...
    # String formatada explicitamente sem os segundos (%H:%M)
    string_hora_minuto = f"{now_br.hour:02d}:{now_br.minute:02d}"
    
    st.markdown(f"""
        <div class='ios-clock-widget'>
            <div class='ios-clock-top'>
                <span class='ios-time'>{string_hora_minuto}</span>
...
with col_tit2:
    renderizar_relogio_realtime_hm_strict()
...
# -----------------------------------------------------------------------------
# 7. PROCESSO DE RENDERIZAÇÃO DOS CARDS OPERACIONAIS (BOTOES COM CORES DOS BALOES)
# -----------------------------------------------------------------------------
def render_cards_operacionais(sub_df, unique_prefix):
    sub_df = sub_df.drop_duplicates(subset=["ordem"], keep="first")

    busca = st.text_input("🔍 Filtrar Ordens", "", placeholder="Filtrar...", key=f"src_v15_{unique_prefix}")
    if busca:
    ...
    modo_exibicao = st.segmented_control("Formato:", options=["Cards Detalhes", "Lista Compacta"], default="Cards Detalhes", key=f"v_v15_{unique_prefix}")
    ...
    for _, row in sub_df.iterrows():
        ...
        # Seletor Interativo Avançado com Cores Iguais aos Balões utilizando type="primary" dinâmico
        col_btn1, col_btn2, col_btn3 = st.columns([1, 1, 1.3])
        
        status_novo = status_atual
        
        with col_btn1:
            # PENDENTE -> Vermelho (#FF453A) quando ativo
            if st.button("🔴 Pendente", key=f"btn_pend_{unique_prefix}_{id_reg}", use_container_width=True, type="primary" if status_atual == "Pendente" else "secondary"):
                status_novo = "Pendente"
        with col_btn2:
            # REALIZADA -> Verde (#30D158) quando ativo
            if st.button("🟢 Realizada", key=f"btn_real_{unique_prefix}_{id_reg}", use_container_width=True, type="primary" if status_atual == "Realizada" else "secondary"):
                status_novo = "Realizada"
        with col_btn3:
            # REPROGRAMAÇÃO -> Laranja (#FF9F0A) quando ativo
            if st.button("🟠 Reprogramar", key=f"btn_reprog_{unique_prefix}_{id_reg}", use_container_width=True, type="primary" if status_atual == "Necessita Reprogramação" else "secondary"):
                status_novo = "Necessita Reprogramação"
                
        # Bloco de comentários reativo
        if status_novo in ["Pendente", "Necessita Reprogramação"]:
            ...
            with col_sel:
                motivo_escolhido = st.selectbox(
                    ...
                    key=f"mot_{unique_prefix}_{id_reg}", label_visibility="collapsed"
                )
            with col_det:
                detalhe_escolhido = st.text_input(
                    "Detalhes", value=detalhe_inicial, placeholder="Observações...", 
                    key=f"det_{unique_prefix}_{id_reg}", label_visibility="collapsed"
                )
            
            # Se mudou o status ou preencheu justificativa, salva
...
if st.session_state.db_data is not None:
    ...
    with aba_geral:
        ...
        if not df_foco_unicas.empty:
            ...
            disc_alvo = st.selectbox("Escolha a disciplina que deseja acompanhar no gráfico:", lista_discs_existentes, key="seletor_reativo_final_v15")
            ...
            st.plotly_chart(fig_barras_horizontal, use_container_width=True, key="graph_v15_core_engine")
    ...
    with aba_exec:
        ...
        exec_sel = st.selectbox("Escolha o Executante da Área:", lista_executantes, key="c_exec_v15")
    ...
    with aba_disc:
        ...
        disc_sel = st.selectbox("Selecione a Disciplina para Filtragem Semanal:", sorted(list(df_foco["disciplina"].dropna().unique())), key="c_disc_page_v15_def")
                ...
                for eng_day, pt_day in dias_mapeamento.items():
                    ...
                    if not df_dia_disc.empty:
                        ...
                        with st.expander(f"{pt_day} — {disc_sel} ({qtd_unicas_disc} Ordens)", expanded=(pt_day.split("-")[0] == hoje_nome_pt)):
                            render_cards_operacionais(df_dia_disc, f"disc_crd_v15_{disc_sel}_{eng_day}")
else:
