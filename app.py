...
<style>
    ...
    /* CARD DE CLIMA - Estilo Gemini Glassmorphism com Transição Suave */
    ...
    /* Widget de Horário e Jornada (Estilo Painel de Operações Semitransparente) */
    .ios-clock-widget { 
        background: #161b22;
...
# -----------------------------------------------------------------------------
# 5. SIDEBAR: FLUID DESIGN (GEMINI INSPIRED CLIMA)
# -----------------------------------------------------------------------------
with st.sidebar:
...
# -----------------------------------------------------------------------------
# 6. HEADER DESIGN CONTEMPORÂNEO (GITHUB CLEAN LOOK)
# -----------------------------------------------------------------------------
...
def render_cards_operacionais(sub_df, unique_prefix):
    ...
    for _, row in sub_df.iterrows():
        ...
        # Controles de interação em grid alinhado com o padrão UI do GitHub
...
if st.session_state.db_data is not None and not st.session_state.db_data.empty:
    ...
    with aba_geral:
        ...
        if not df_foco_unicas.empty:
            ...
            fig_barras_horizontal.add_trace(fig_barents)
