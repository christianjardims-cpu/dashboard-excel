...
if df is not None:
    ...
    with aba_execucao:
        ...
        # Selecionando colunas chave para visualização amigável na tabela editável
        colunas_exibicao = ["Ordem", "Descrição da Ordens", "Texto Breve da Operação", "Executante", "Disciplina", "Data de Início", "Status_Execucao"]
        df_edicao = df_filtrado[colunas_exibicao].copy()
        
        # st_data_editor permite edição em linha
        ...
        if st.button("Salvar Alterações no Banco de Dados"):
            for idx, row in df_atualizado.iterrows():
                ...
                # Atualiza o dataframe original pelo índice correspondente
