...
# Injeção de CSS - Tema Escuro Minimalista (Estilo Apple Dark Mode / macOS)
st.markdown("""
<style>
...
st.markdown("<p style='color: #8E8E93; margin-top: -10px;'>Programação Semanal de Manutenção</p>", unsafe_allow_html=True)
...
if df is not None:
    ...
    with aba_geral:
        ...
        with col_g1:
            ...
            fig_area = px.bar(resumo_area, x="Área", y="% Realizado", text_auto=True, color="Área", color_discrete_sequence=["#0A84FF", "#5E5CE6"])
            fig_area.update_layout(template="plotly_dark", margin=dict(t=20, b=20), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
        ...
        with col_g2:
            ...
            fig_pizza = px.pie(status_geral, values="Quantidade", names="Status", hole=0.5, color_discrete_sequence=["#30D158", "#FF9F0A", "#FF453A"])
            fig_pizza.update_layout(template="plotly_dark", margin=dict(t=20, b=20), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
        ...
        fig_disc = px.bar(resumo_disc, x="Disciplina", y=["Realizada", "Pendente", "Necessita Reprogramação"], barmode="group", color_discrete_sequence=["#0A84FF", "#FF9F0A", "#FF453A"])
        fig_disc.update_layout(template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
    ...
    with aba_execucao:
        ...
        if exec_sel != "Selecione...":
            ...
            for dia in dias_da_semana_ordem:
                ...
                if not df_dia.empty:
                    st.divider()
                    st.markdown(f"<h4 style='font-weight: 500; color: #0A84FF;'>📅 {dias_pt[dia]}</h4>", unsafe_allow_html=True)
    ...
    st.warning("⬅️ Por favor, insira a senha e faça o upload do arquivo base no menu lateral para inicializar.")