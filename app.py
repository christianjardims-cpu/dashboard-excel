...
if df is not None:
    ...
    colunas_necessarias = ["Área", "Executante", "Ordem", "Descrição da Ordens", "Texto Breve da Operação", "Duração", "Data de Início"]
    
    # Encontrar coluna de área considerando variações
    coluna_area = None
    for col in df.columns:
        if "área" in col.lower() or "area" in col.lower():
            coluna_area = col
            break

    if not coluna_area:
        st.error("Não foi possível encontrar a coluna de 'Área' na sua planilha. Verifique o arquivo.")
    else:
        ...
        with col_filtro1:
            areas = ["Todas"] + sorted([str(a) for a in df[coluna_area].dropna().unique()])
        ...
        if area_selecionada != "Todas":
            df_filtrado = df[df[coluna_area] == area_selecionada]
        else:
            df_filtrado = df
            ...
            for index, row in df_exec.iterrows():
                ...
                with st.container(border=True):
                    ...
                    with col_d3:
                        ...
                        st.session_state.status_ordens[ordem] = status
