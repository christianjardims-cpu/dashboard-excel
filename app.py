...
# Mapeamento de Cores Atualizado: Pendente (Vermelho), Realizada (Verde), Necessita Reprogramação (Laranja)
COLOR_MAP = {
        ...
        if uploaded_file is not None:
            ...
            try:
                with st.spinner("Processando base de dados..."):
                    ...
                    # Forçando tipagem para string logo na criação
...
def render_cards_com_busca(sub_df, prefix_key):
    ...
    for idx, row in df_filtrado_busca.iterrows():
        ...
        # Garante segurança ao ler comentário como string
        comentario_actual = str(row_actual.get("Comentario", ""))
        if comentario_actual == "nan": 
            comentario_actual = ""
        ...
        if novo_status in ["Pendente", "Necessita Reprogramação"]:
            ...
            if comentario_atual and comentario_atual != "None":
